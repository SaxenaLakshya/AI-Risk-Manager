...
export type SubmitOrderResponse = ScoreResponse & {
  request_id: string | null;
  queued_for_review: boolean;
};

export async function submitOrder(order: OrderInput): Promise<SubmitOrderResponse> {
  const res = await fetch(`${API_URL}/submit-order`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(order),
  });
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`submit-order failed: ${res.status} ${err}`);
  }
  return res.json();
}
...
export async function getPendingRequests(): Promise<PendingRequest[]> {
  const res = await fetch(`${API_URL}/pending-requests`, { cache: "no-store" });
  if (!res.ok) {
    throw new Error(`pending-requests failed: ${res.status}`);
  }
  return res.json();
}
